...
CXX        = emcc 
CC         = $(CXX)
CCFLAGS    = -std=c++20 -O3 -Wall -Wfatal-errors -fexceptions -s USE_SDL=2 -g
...
LFLAGS= -s USE_SDL=2 $(WASM_EXPORTS) -s DISABLE_DEPRECATED_FIND_EVENT_TARGET_BEHAVIOR=1 -s NO_DISABLE_EXCEPTION_CATCHING -s LLD_REPORT_UNDEFINED -g -s ASSERTIONS=2  -s SAFE_HEAP=1 
#-s ASSERTIONS=0 -s GL_ASSERTIONS=0 
...
bin:
	@echo "Linking"
	$(CC) $(LFLAGS) -o vAmiga.html --shell-file shell.html  -s INITIAL_MEMORY=256MB -s TOTAL_STACK=32MB  *.o build/*.a build/*/*.a
#	$(CC) $(LFLAGS) -o vAmiga.html --shell-file shell.html  -s INITIAL_MEMORY=256MB -s TOTAL_STACK=32MB  *.o emulator.a
	#--preload-file roms
